...
def sum_by_group(
    ...
    *values: npt.NDArray,
    ...
    :return: `unique_groups, (grouped_sum_1, grouped_sum_2, ...)`
        Sorted unique group labels (as ints).
        Summed values for each input array, aligned with unique_groups.

    .. warning:: We use `np.bincount` which creates an array of `groups.max() + 1` size,
    so having large numbers in the `groups` array is disadvantageous.

    .. note:: NaNs are propogated: if an item of some value array is NaN, the sum of this value
    for a corresponding group will be NaN
    """
    if groups.size == 0:
        return np.empty(0, dtype=int), [np.empty(0, dtype=val.dtype) for val in values]
    groups = groups.astype(int)
    unique_groups = np.flatnonzero(np.bincount(groups))
    grouped_sums = [
